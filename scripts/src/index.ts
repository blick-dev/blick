...
export class Scripts {
  ...
  async init(@param(name) n, @param(appID) id, @context() ctx: Context) {
    ...
    await this.exec(`rm -rf ${ctx.directory}/../electron`, true);
    await this.exec(
      `cd ${ctx.directory}/../ && npm install && ng run app:build`,
      true
    );
  ...
  async build(
    ...
    await this.exec(
      `cd ${ctx.directory}/../ && ng run app:build --base-href ./${
        production ? " --prod" : ""
      }`,
      true
    );
    ...
    if (plt === "android") {
      await this.exec(
        `cd ${ctx.directory}/../android/ && ./gradlew build`,
        true
      );
    } else if (plt === "ios") {
  ...
  async install(
    ...
    if (plt === "android") {
      const apk = production
        ? "release/app-release-unsigned.apk"
        : "debug/app-debug.apk";
      await this.exec(
        `native-run android --app ${ctx.directory}/../android/app/build/outputs/apk/${apk}`,
        true
      );
    } else if (plt === "electron") {
      await this.exec(
        `cd ${ctx.directory}/../electron &&  npx electron ./`,
        true
      );
    } else if (plt === "ios") {
      // not yet implemented
  ...
  async screenshots(@context() ctx: Context, @param(headless) headl = true) {
    ...
    for (const device of devices) {
      ...
      for (const r of routes) {
        ...
        await page.goto(`http://localhost:8100/${r.path}`, {
          waitUntil: "networkidle2"
        });
        await page.screenshot({
          path: `${ctx.directory}/../screenshots/${device.name}_${r.name}.png`
        });
      }
      spinner.stop();
  ...
  async updatePackageJSON(ctx: Context, appName?: string) {
    const packageJSON = await this.parseJSON(
      `${ctx.directory}/../package.json`
    );
  ...
  @Action({
    addToHistory: true,
    description: (ctx, appName) =>
      `Updated capacitor config with name ${appName}`
  })
  async updateCapacitorConfig(ctx: Context, appName: string, id: string) {
    const capacitorConfig = await this.parseJSON(
      `${ctx.directory}/../capacitor.config.json`
    );
  ...
  async updateFastlaneFile(
    ...
    const fileContents = await this.readFile(
      `${ctx.directory}/../fastlane/${file}`
    );
    await this.writeFile(
      `${ctx.directory}/../fastlane/${file}`,
      fileContents.replace(new RegExp("de.fivethree.cap", "g"), id)
    );
  }
  ...
  async onError(@context() ctx: Context, @error() err) {
    console.log(
      "Error happenend while executing command",
      ctx.api.getLatestHistoryEntry().latest.name,
      err
    );
  }
